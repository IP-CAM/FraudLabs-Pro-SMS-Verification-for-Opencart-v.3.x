...
class ControllerExtensionFraudFraudLabsProSmsVerification extends Controller {
	...
	public function before_checkout_success(){
		// Do not perform fraud SMS verification if FraudLabs Pro SMS Verification is disabled.
		if (!$this->config->get('fraud_fraudlabsprosmsverification_status')) {
			$this->write_debug_log('FraudLabs Pro SMS Verification is disabled. SMS Verification will not be performed.');
			return;
		...
		if (isset($this->session->data['order_id'])) {
			...
			if ($flpStatus == 'REVIEW') {
				...
				if (filter_var($emailAddress, FILTER_VALIDATE_EMAIL)) {
					...
					$link = $_SERVER["SERVER_NAME"] . '/index.php?route=extension/fraud/fraudlabsprosmsverification&orderid=' . $orderId . '&code=' . $code;
